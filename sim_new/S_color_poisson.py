...
def self_consistency(z,k,phi):
    return 1.-phi+phi*gen(z,k)
...
def analytics(k_av,r_c,nr_sam):
    # some needed quantities
    C=len(r_c[:])
    # calculate giant component
    phi=1.
    u0=opt.fixed_point(self_consistency,0.5,args=[k_av,phi])
    ...
    for k1 in arange(50)+2:
        ...
        for i_rand in arange(nr_sam):
            ...
            S_color_analyt+=poisson.pmf(k1,k_av)*p_success/nr_sam
...
ks=linspace(1,10,91)
#ks=linspace(1,2,21)
#ks=linspace(4./3,4./3+.5,10)
C=4
#k_crit_1=1.4
#ndeg=1
#r_c_1=1.-1./k_crit_1
#r_c=append(r_c_1,(1.-r_c_1)/(C-1)*ones(C-1))
r_c=1./C*ones(C)
runs=100
...
if analytical:
    nr_sam=100
#    fname='../data_new/S_color_poisson_analyt_C%d_kc%07.4f_ndeg%d_n%d.dat'%(C,k_crit_1,ndeg,nr_sam)
...
for i in arange(runs):
    ...
    for k_av in ks:
        ...
        if analytical:
            ...
            ### color distribution
            ### analytics
            analyt=analytics(k_av,r_c,nr_sam)
        ...
        if not analytical:
            ### prepare graphs
            g = gt.random_graph(N, lambda: poisson.rvs(k_av), directed=False)
            c = g.new_vertex_property('int')
            degs=g.degree_property_map('out').a
            for n_ in arange(N):
                c.a[n_]=C-sum(1*(random()<cumsum(r_c)))
            ### count component
            numer_=numerics(g,c)
            print('k_av numerics:',k_av,numer_)
            ### save to file
            file_=open(fname,'a')
            file_.write('%e %e\n'%(k_av,numer_))
            file_.close()
